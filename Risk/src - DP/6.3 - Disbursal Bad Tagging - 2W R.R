...
load_rdata_intermediate("ADS_data//disbursals_final_2W_RF.rdata")
...
tag_bad_loan <- function(mob_timeframe,DPD_value_cutoff,minDate,maxDate,temp_loan_type){
  ...
  # disbursals_with_enough_datapoints <- distinct(temp_month_count %>% filter(month_count >= 1 & month_count <= mob_timeframe) %>% ungroup() %>% dplyr::select(deal_no))
...
# minDate = 201803
# maxDate = 201812
# temp_loan_type = 'Refinance-2W'

## 3.1 tag bad loans in 2W New
bad_loans_2W_RF <- tag_bad_loan(mob_timeframe = 15,
                                 DPD_value_cutoff = 60,
                                 minDate = 201803, 
                                 maxDate = 201812,
                                 temp_loan_type = 'Refinance-2W')

bad_loans <- bad_loans_2W_RF %>% ungroup()
...
save(bad_loans,
     file = file.path(
       ...
       "ADS_data",
       "bad_loans_2W_RF.rdata"
     )
)

# save(bad_loans,
#      file = file.path("data//intermediate//ADS_data//bad_loans_2W_RF_LE15mob.rdata"
#      )
# )
...
v1_2W_RF <- tag_bad_loan(mob_timeframe = 12,
                          DPD_value_cutoff = 60,
                          minDate = 201901, 
                          maxDate = 201903,
                          temp_loan_type = 'Refinance-2W')
...
validation_60dpd_12mob <- data.frame(v1_2W_RF)
...
save(validation_60dpd_12mob,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "validation_60dpd_12mob_2W_RF.rdata"
     )
)

# save(validation_60dpd_12mob,
#      file = file.path("data//intermediate//ADS_data//validation_60dpd_12mob_2W_RF.rdata"
#      )
# )

rm(validation_60dpd_12mob, v1_2W_RF)



## validation datasets - 60dpd in 9mob
v2_2W_RF <- tag_bad_loan(mob_timeframe = 9,
                          DPD_value_cutoff = 60,
                          minDate = 201904, 
                          maxDate = 201906,
                          temp_loan_type = 'Refinance-2W')
...
validation_60dpd_9mob <- data.frame(v2_2W_RF)
...
save(validation_60dpd_9mob,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "validation_60dpd_9mob_2W_RF.rdata"
     )
)

# save(validation_60dpd_9mob,
#      file = file.path("data//intermediate//ADS_data//validation_60dpd_9mob_2W_RF.rdata"
#      )
# )

rm(v2_2W_RF,validation_60dpd_9mob)
...
##########################################################################################################
load_rdata_intermediate("ADS_data//disbursals_final_all_2W_RF.rdata")
...
v3_2W_RF <- tag_bad_loan_in_specfic_mob(mob_timeframe = 12,
                          DPD_value_cutoff = 60,
                          minDate = 201910,
                          maxDate = 201912,
                          temp_loan_type = 'Refinance-2W',
...
validation_60dpd_oct_to_dec_2020 <- data.frame(v3_2W_RF)
...
save(validation_60dpd_oct_to_dec_2020,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "validation_60dpd_oct_to_dec_2020_2W_RF.rdata"
     )
)
