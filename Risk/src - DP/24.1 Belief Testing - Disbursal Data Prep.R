...
asset_classification_data <- distinct(asset_classification_data %>% dplyr::select(Deal_No,Segment_Code))
colnames(asset_classification_data) <- c('deal_no',
                                         'asset_tag')

asset_list <- c("MOTORCYC",
"HISTREET",
"SCOOTER",
"MOPEDS",
"ACAR",
...
for (asset in asset_list){
  new_col <- paste0("Rule_Asset_",asset)
  asset_classification_data[,new_col] <- as.numeric(ifelse(asset_classification_data$asset_tag == asset, 1, 0))
}

asset_classification_data$Rule_Asset_OTHERS <- as.numeric(ifelse(asset_classification_data$asset_tag %notin% asset_list, 1, 0))
...
all_rules <- left_join(all_rules, asset_classification_data, by = c('deal_no'))

rm(final_output,rules_DL,rules_PO,rules_CV_Demog, asset_classification_data, asset_list)
...
save(belief_testing_rules_disbursals,
     file = file.path(
       ...
       "belief_testing",
       "all_rules",
       "belief_testing_rules_disbursals.rdata"
     )
)

# save(belief_testing_rules_disbursals,
#      file = file.path("data//intermediate//belief_testing//all_rules//belief_testing_rules_disbursals_new.rdata"
#      )
# )
