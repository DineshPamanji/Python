...
# load_rdata_intermediate("ADS_data//bureau_tradelines.rdata")
# load_rdata_intermediate("ADS_data//bureau_tradelines_vf.rdata")
# load_rdata_intermediate("ADS_data//bureau_tradelines_vcheck.rdata")
load_rdata_intermediate("ADS_data//bureau_tradelines_dpd_check.rdata")
...
# count_custid <- trades_clean %>% group_by(deal_no) %>% summarise(count = length(unique(customer_code)))
...
rules_PO <- distinct(tradelines %>% dplyr::select(deal_no,customer_code,applicant_id,loan_type))

for(temp_block in blocks_temp){
  temp_rules <- req_cols[min(temp_block) : max(temp_block)]
  temp_agg <- tradelines %>% dplyr::select(c('deal_no','customer_code','applicant_id', 'loan_type',temp_rules))
  temp_agg <- temp_agg %>% group_by(deal_no,customer_code,applicant_id,loan_type) %>% summarise(across(everything(), list(sum)))
  colnames(temp_agg) <- c('deal_no', 'customer_code','applicant_id','loan_type', temp_rules)
  temp_agg <- temp_agg %>% ungroup()
  
  rules_PO <- left_join(rules_PO,temp_agg,by = c('deal_no' = 'deal_no',
...
rules_PO_agg$customer_code <- NULL
...
temp_output_df <- distinct(rules_PO_agg %>% dplyr::select(deal_no, loan_type))

for(temp_block in blocks_temp){
  temp_rules <- rule_names[min(temp_block) : max(temp_block)]
  temp_agg <- rules_PO_agg %>% dplyr::select(c('deal_no', 'loan_type',temp_rules))
...
# load_rdata_intermediate("ADS_data//trades_melt.rdata")
# load_rdata_intermediate("ADS_data//trades_melt_vf.rdata")
# load_rdata_intermediate("ADS_data//trades_melt_vcheck.rdata")
load_rdata_intermediate("ADS_data//trades_melt_dpd_check.rdata")
...
rules_DL <- distinct(trades_dpd %>% dplyr::select(deal_no,customer_code,applicant_id,loan_type))

for(temp_block in blocks_temp){
  temp_rules <- req_cols[min(temp_block) : max(temp_block)]
  temp_agg <- trades_dpd %>% dplyr::select(c('deal_no','customer_code','applicant_id', 'loan_type',temp_rules))
  temp_agg <- temp_agg %>% group_by(deal_no,customer_code,applicant_id,loan_type) %>% summarise(across(everything(), list(sum)))
  colnames(temp_agg) <- c('deal_no', 'customer_code','applicant_id','loan_type', temp_rules)
  temp_agg <- temp_agg %>% ungroup()
...
# 
# trades_dpd_p1 <- trades_dpd %>% dplyr::select(c('deal_no', 'customer_code', 'applicant_id', 'loan_type', req_cols_p1))
# trades_dpd_p2 <- trades_dpd %>% dplyr::select(c('deal_no', 'customer_code', 'applicant_id', 'loan_type', req_cols_p2))
# 
# rm(trades_dpd)
# ## 3.7 aggregate rules at deal number level
# rules_DL_p1 <- trades_dpd_p1 %>% group_by(deal_no,customer_code,applicant_id,loan_type) %>% summarise(across(everything(), list(sum)))
# rm(trades_dpd_p1)
# 
# 
# 
# rules_DL_p2 <- trades_dpd_p2 %>% group_by(deal_no,customer_code,applicant_id,loan_type) %>% summarise(across(everything(), list(sum)))
# rm(trades_dpd_p2)
# 
# colnames(rules_DL_p1) <- c('deal_no', 'customer_code', 'applicant_id', 'loan_type', req_cols_p1)
# rules_DL_p1 <- rules_DL_p1 %>% ungroup()
# 
# colnames(rules_DL_p2) <- c('deal_no', 'customer_code', 'applicant_id', 'loan_type', req_cols_p2)
...
# load_rdata_intermediate("ADS_data//enquiry_data_vf.rdata")
load_rdata_intermediate("ADS_data//enquiry_data_vcheck.rdata")
...
colnames(rules_EN)[colnames(rules_EN) == 'Customer_Code'] <- 'customer_code'
...
# load_rdata_output("gating_rules//rules_PO_agg.rdata")
# load_rdata_output("gating_rules//rules_DL_agg.rdata")
# load_rdata_output("gating_rules//rules_EN_agg.rdata")

# load_rdata_output("gating_rules//rules_PO_agg_vcheck.rdata")
# load_rdata_output("gating_rules//rules_DL_agg_vcheck.rdata")
# load_rdata_output("gating_rules//rules_EN_agg_vcheck.rdata")


load_rdata_output("gating_rules//rules_PO_agg_dpd_check.rdata")
load_rdata_output("gating_rules//rules_DL_agg_dpd_check.rdata")
load_rdata_output("gating_rules//rules_EN_agg_vcheck.rdata")
...
#        get_data_path()$data$output,
#        "gating_rules",
#        "gating_rules_deal_no_level_vcheck.rdata"
#      )
# )
# 
...
save(gating_rules_deal_no_level,
     file = file.path(
       get_data_path()$data$output,
       "gating_rules",
       "gating_rules_deal_no_level_dpd_check.rdata"
     )
)
...
output <- gating_rules_deal_no_level
...
rules_agg_2W_new <- calculate_rule_statistics(gating_rules_data = output, loan_type_temp = '2W-New')

save_csv_output(rules_agg_2W_new, paste0("\\gating_rules\\post_workshop\\gating_rules_2W-New.csv"))


## 7.3 get rule statistics for PV New
rules_agg_PV_new <- calculate_rule_statistics(gating_rules_data = output, loan_type_temp = 'PV-New')

save_csv_output(rules_agg_PV_new, paste0("\\gating_rules\\post_workshop\\gating_rules_PV-New.csv"))


## 7.4 get rule statistics for PV Used
rules_agg_PV_used <- calculate_rule_statistics(gating_rules_data = output, loan_type_temp = 'PV-Used')

save_csv_output(rules_agg_PV_used, paste0("\\gating_rules\\post_workshop\\gating_rules_PV-Used_dpd_check.csv"))
