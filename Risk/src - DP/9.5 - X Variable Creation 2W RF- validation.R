...
load_rdata_intermediate("ADS_data//trades_melt_validation_2W_RF.rdata")
...
load_rdata_intermediate("ADS_data//enquiry_data_validation_2W_RF.rdata")
...
create_EV_x_enq_y_mon <- function(enquiry_data, months_vector, buffer){
  ...
  for(month_value in months_vector){
    ...
    # 2W
    enquiry_agg <- enquiry_agg %>% filter((days > (0+buffer_2W_RF)) & (days <= (days_cutoff+buffer_2W_RF))) %>% group_by(deal_no,
                                                                                                                                 Appno,
                                                                                                                                 applicant_id,
...
create_EV_unsec_x_enq_y_mon <- function(enquiry_data, months_vector,buffer){
  ...
  for(month_value in months_vector){
    ...
    # get count of enquiries at deal no x customer code x loan notation level
    # subset for loan types
    enquiry_agg <- enquiry_data_unsec %>% filter(loan_type == 'Refinance-2W')
...
save(all_X_var_bureau,
     file = file.path(
       ...
       "ADS_data",
       "X_var_bureau_validation_2W_RF.rdata"
     )
)

# save(all_X_var_bureau,
#      file = file.path("data//intermediate//ADS_data//X_var_bureau_validation_2W_RF.rdata"
#      )
# )
...
load_rdata_intermediate("ADS_data//X_var_bureau_validation_2W_RF.rdata")
...
#model_data_2W_RF1 <- model_data_2W_RF %>% filter(Appno %in% unique(primary_app_customer_code$customer_code))
model_data_2W_RF <- left_join(model_data_2W_RF,validation_data,by='deal_no')

rm(all_X_var_application,all_X_var_bureau,validation_data,base_ads)
...
save(model_data_2W_RF,
     file = file.path(
       get_data_path()$data$intermediate,
       "model_data",
       "model_data_2W_RF_validation.rdata"
     )
)
# save(model_data_2W_RF,
#      file = file.path("data//intermediate//model_data//model_data_2W_RF_validation.rdata"
#      )
# )
