...
subset2 <- dev_data %>% filter(Region == 'West')
subset3 <- dev_data %>% filter(Region == 'North')
subset4 <- dev_data %>% filter(Region == 'Central' | Region == 'East')
...
dev_data$region_woe <- ifelse(dev_data$Region == 'South', woe1, 
                              ifelse(dev_data$Region == 'West', woe2,
                                     ifelse(dev_data$Region == 'North', woe3, woe4)))

oot_data$region_woe <- ifelse(oot_data$Region == 'South', woe1, 
                              ifelse(oot_data$Region == 'West', woe2,
                                     ifelse(oot_data$Region == 'North', woe3,woe4)))
...
dev_data$phones_reported_3m_C1 <- ifelse(dev_data$phones_reported_3m_C >= 4, 4, 
                                         ifelse(dev_data$phones_reported_3m_C <= 0, 0, dev_data$phones_reported_3m_C))
oot_data$phones_reported_3m_C1 <- ifelse(oot_data$phones_reported_3m_C >= 4, 4, 
                                         ifelse(oot_data$phones_reported_3m_C <= 0, 0, oot_data$phones_reported_3m_C))
...
## transformation
dev_data$phones_reported_6m_C1 <- ifelse(dev_data$phones_reported_6m_C >= 4, 4, 
                                         ifelse(dev_data$phones_reported_6m_C <= 0, 0, dev_data$phones_reported_6m_C))
oot_data$phones_reported_6m_C1 <- ifelse(oot_data$phones_reported_6m_C >= 4, 4, 
...
## transformation
dev_data$addresses_reported_6m_C1 <- ifelse(dev_data$addresses_reported_6m_C >= 3, 3, 
                                            ifelse(dev_data$addresses_reported_6m_C <= 1, 1, dev_data$addresses_reported_6m_C))
oot_data$addresses_reported_6m_C1 <- ifelse(oot_data$addresses_reported_6m_C >= 3, 3, 
...
## transformation
dev_data$addresses_reported_12m_C1 <- ifelse(dev_data$addresses_reported_12m_C >= 3, 3, 
                                             ifelse(dev_data$addresses_reported_12m_C <= 0, 0, dev_data$addresses_reported_12m_C))
oot_data$addresses_reported_12m_C1 <- ifelse(oot_data$addresses_reported_12m_C >= 3, 3, 
...
subset2 <- dev_data %>% filter(asset_tag %notin% c('AMUV','BCAR','ACAR'))
subset3 <- dev_data %>% filter(asset_tag == 'ACAR')
subset4 <- dev_data %>% filter(asset_tag == 'BCAR')
...
dev_data$asset_tag_woe <- ifelse(dev_data$asset_tag == 'AMUV', woe1, 
                                 ifelse(dev_data$asset_tag == 'ACAR', woe3,
                                        ifelse(dev_data$asset_tag == 'BCAR', woe4, woe2)))

oot_data$asset_tag_woe <- ifelse(oot_data$asset_tag == 'AMUV', woe1, 
                                 ifelse(oot_data$asset_tag == 'ACAR', woe3,
                                        ifelse(oot_data$asset_tag == 'BCAR', woe4, woe2)))
...
seed_iterations_output <- get_seed_iterations_no_test(seed_list = c(1:100),
                                              dev_data = dev_data,
                                              oot = oot_data,
...
save_xlsx_output(data = output_list, relative_path = "//model//PV_Used//NTC//vcheck//Model_summary_PV_Used_NTC.xlsx")
 # write_xlsx(output_list, path = "data//output//model//PV_Used//NTC//vcheck//Model_summary_PV_Used_NTC_v2.xlsx")


model_PV_NTC <- final_model
saveRDS(model_PV_NTC, file = file.path(get_data_path()$data$model,"//vcheck//model_PV_Used_NTC.rds"))
 # saveRDS(model_PV_NTC, file = file.path("data//output//model//PV_Used//NTC//vcheck//model_PV_Used_NTC.rds"))
...
## Load PV New model
final_model <- readRDS("data/output/model/PV_New/NTC/vcheck/model_PV_New_NTC.rds")
performance_data <- get_predictions(final_model, dev_data)

model_performance <- get_model_performance_no_test(performance_data, performance_data)


# 6.8 get rank ordering for 60 dpd in 12mob
RO_Decile <-  data.frame(get_RO_validation_decile(performance_data))
RO_Pentile <-  data.frame(get_RO_validation_pentile(performance_data))
...
output_list <- list("model_summary" = data.frame(tidy(final_model)),
                    ...
                    "VIF" = get_vif(final_model),
                    "wald_chi_sq" = get_wald_chi_sq(final_model),
                    "RO_overall_D" = RO_Decile,
                    "RO_overall_P" = RO_Pentile
)

write_xlsx(output_list, path = "data//output//model//PV_Used//NTC//vcheck//Model_summary_PV_Used_NTC_v5.xlsx")

train_data_final <- performance_data
...
temporal_decay_RO_decile <- data.frame(get_RO_validation_decile(temporal_decay_train))
temporal_decay_RO_pentile <- data.frame(get_RO_validation_pentile(temporal_decay_train))
...
save_xlsx_output(data = output_list, relative_path = "//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_1-Temporal decay.xlsx")
# write_xlsx(output_list, path = "data//output//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_1-Temporal decay.xlsx")
...
ever_90dpd_RO_decile <- data.frame(get_RO_validation_decile(ever_train))
ever_90dpd_RO_pentile <- data.frame(get_RO_validation_pentile(ever_train))
...
ever_150dpd_RO_decile <- data.frame(get_RO_validation_decile(ever_train))
ever_150dpd_RO_pentile <- data.frame(get_RO_validation_pentile(ever_train))
...
save_xlsx_output(data = output_list, relative_path = "//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_2-Ever 90 150 DPD.xlsx")
# write_xlsx(output_list, path = "data//output//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_2-Ever 90 150 DPD.xlsx")
...
model_data <- ntc_model_data %>% filter(loan_type == 'PV-Used')
...
all_validation <- all_validation %>% filter(loan_type == 'PV-Used')
...
save_xlsx_output(data = output_list, relative_path = "//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_3-OOT.xlsx")
# write_xlsx(output_list, path = "data//output//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_5-OOT.xlsx")
...
dev_data <- inner_join(performance_data,cibil_ntc)




## 4. remove NA & -1 cibil scores
dev_data <- dev_data %>% filter(!is.na(cibil_ntc_score) & cibil_ntc_score != -1)
...
write_xlsx(output_list, path = file.path("data//output//model//PV_Used//NTC//vcheck//Model_validation_PV_Used_1-CIBIL_NTC_crosstab_latest.xlsx"))
save(model_data_PV_Used_ntc, file = file.path("data//output//model//PV_Used//NTC//vcheck//model_data_pv_used_ntc.rdata"))
