...
load_rdata_intermediate("cleaned_data//retro_as_on_data_reject.rdata")

retro_as_on_data <- data.frame(retro_as_on_data_reject)
...
load_rdata_intermediate("reject_data//reject_data_2W_RF.rdata")

## 2. Subset data & create unique loan identifier ------------------------------------------

## 2.1 subset disbursal data
retro_as_on_data$year_mon <- paste0(year(retro_as_on_data$application_date), "-", str_pad(month(retro_as_on_data$application_date),2,"0",side = "left"))

retro_as_on_data <- distinct(retro_as_on_data %>% filter(application_no %in% unique(c(reject_data_2W$application_no,
                                                                                      reject_data_PV_New$application_no,
                                                                                      reject_data_PV_Used$application_no,
                                                                                      reject_data_2W_RF$application_no
                                                                                      
                                                                                      ))))


rm(reject_data_2W, reject_data_PV_New,reject_data_PV_Used,reject_data_2W_RF, retro_as_on_data_reject)

## 2.3 clean tradeline data
tradeline_subset <- retro_as_on_data %>% filter((!is.na(high_credit_sanctioned_amount)) &
...
tradeline_subset$applicant_id <- paste0(tradeline_subset$application_no, "_", tradeline_subset$customer_code)
rm(retro_as_on_data)
...
not_duplicated_data <- not_duplicated_data %>% ungroup()
...
save(trades_clean,
     file = file.path(
       ...
       "reject_data",
       "pre_app_bureau_tradelines_rejects.rdata"
     )
)
...
save(trades_melt,
     file = file.path(
       get_data_path()$data$intermediate,
       "reject_data",
       "pre_app_trades_melt_rejects.rdata"
     )
)

rm(trades_melt,year_mon_index,month_dpd_mapping)
...
## 7.1 load deal numbers for which enquiry data is needed 
load_rdata_intermediate("reject_data//reject_data_2W.rdata")
load_rdata_intermediate("reject_data//reject_data_PV_New.rdata")
load_rdata_intermediate("reject_data//reject_data_PV_Used.rdata")
...
rm(reject_data_2W,reject_data_PV_New,reject_data_PV_Used)

## 7.2 load enquiry data
load_rdata_intermediate("cleaned_data//reject_enquiry.rdata")


# 7.3 subset enquiry data
enquiry_data <- inner_join(reject_enquiry, reject_data_all, by = c("Application_No" = "application_no"))
# enquiry_data <- inner_join(reject_enquiry, reject_data_2W, by = c("Application_No" = "application_no"))

## 7.4 get cibil reported date for deal numbers
trades_clean <- distinct(trades_clean %>% dplyr::select(application_no,application_date,cibil_reported_date))
...
save(enquiry_data,
     file = file.path(
       get_data_path()$data$intermediate,
       "reject_data",
       "pre_app_enquiry_data_rejects.rdata"
     )
)
rm(list=ls())
