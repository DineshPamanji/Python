...
rules_DL <- trades_dpd %>% group_by(application_no, applicant_id) %>% summarise(across(everything(), list(sum)))

colnames(rules_DL) <- c('application_no', 'applicant_id', req_cols)
...
rules_PO <- tradelines %>% dplyr::select(c('application_no', 'applicant_id',req_cols))

## 3.11 Aggregate to applicant level
rules_PO <- rules_PO %>% group_by(application_no, applicant_id) %>% summarise(across(everything(), list(sum)))
...
get_exposure_variables <- function(tradelines){
  ...
  exposure_var <- exposure_var %>% filter(loan_notation == 'CC') %>% group_by(application_no, applicant_id) %>% summarise(
...
all_X_var_bureau <- all_X_var_bureau_rejects %>% dplyr::select(columns_list)
rm(all_X_var_bureau_rejects)
...
rules_PO <- left_join(rules_PO, all_X_var_bureau,by = c('application_no', 'applicant_id'))
rules_PO <- left_join(rules_PO, exposure_df,by = c('application_no', 'applicant_id'))
...
rules_PO <- left_join(rules_PO,product_mapping,by = 'application_no')
...
# rules_PO_2W <- rules_PO %>% filter(product == 'H')
# rules_PO_PV <- rules_PO %>% filter(product == 'C')
# 
# 
# 
...
cv_demog$ECN <- as.numeric(cv_demog$ECN)

## 7.2 Load ECN mapping
load_rdata_intermediate("cleaned_data//cibil_v3_applications.rdata")
cibil_v3_applications$cibil_score_v3 <- NULL
  
  
## 7.3 join data
cv_demog_data <- inner_join(cibil_v3_applications,cv_demog,by = c('EnquiryControlNumber'='ECN'))
rm(cibil_v3_applications,cv_demog)

cv_demog_data <- data.frame(cv_demog_data %>% filter(Application_No %in% unique(final_output$application_no)))


## 7.4 select required columns
cv_demog_data <- cv_demog_data %>% dplyr::select(Application_No,
                                                 phones_reported_3m,
                                                 phones_reported_6m,
                                                 phones_reported_12m,
...
all_rules <- left_join(final_output,rules_PO, by = c('application_no','applicant_id'))

all_rules <- left_join(all_rules, rules_DL, by = c('application_no','applicant_id'))

all_rules <- left_join(all_rules, rules_CV_Demog, by = c('application_no' = 'Application_No'))
...
belief_testing_rules_applications <- all_rules
assert_data_non_empty(belief_testing_rules_applications)
save(belief_testing_rules_applications,
     file = file.path(
       get_data_path()$data$intermediate,
       "belief_testing",
       "all_rules",
       "belief_testing_rules_applications.rdata"
     )
)
