...
rm(disbursal_data,retro_as_on_data, bad_loans )
...
account_mapping <- fread_mapping("account_type_mapping.csv")
 # account_mapping <- fread("data//mapping//account_type_mapping.csv")
...
save(trades_clean,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "bureau_tradelines_2W_RF_modeling.rdata"
     )
)

# save(trades_clean,
#      file = file.path(
#        "data//intermediate//ADS_data//bureau_tradelines_2W_RF_modeling.rdata"
#      )
# )
...
# fwrite(trades_clean,"cibil_bureau_data_2W_Refinance.csv", row.names = F)
...
save(trades_melt,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "trades_melt_2W_RF_modeling.rdata"
     )
)

# save(trades_melt,
#      file = file.path("data//intermediate//ADS_data//trades_melt_2W_RF_modeling.rdata"
#      )
# )
...
# fwrite(trades_melt,"cibil_bureau_data_2W_Refinance_melt.csv", row.names = F)
...
enquiry_data <- enquiry_data_2W_RF %>% dplyr::select(colnames(enquiry_data_2W_RF))
...
## Need help here from Vishnu
trades_clean <- distinct(trades_clean %>% dplyr::select(deal_no,Appno,applicant_id,disbursal_date,cibil_reported_date))
...
save(enquiry_data,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "enquiry_data_vf_2W_RF.rdata"
     )
)

# save(enquiry_data,
#      file = file.path("data//intermediate//ADS_data//enquiry_data_2W_RF_modeling.rdata"
#      )
# )

# enquiry_data_2W_RF_modeling.rdata
rm(list=ls())

fwrite(enquiry_data,"cibil_enquiry_data_2W_Refinance.csv", row.names = F)
