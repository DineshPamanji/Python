...
dark_green_band <- c('1-1','1-2','2-1','2-2')
light_green_band <- c('3-1','3-2','3-3','1-3','2-3','3-3')
orange_band <- c('2-5','2-6','3-4','3-5','3-6','4-4','5.1-4','5.2-3')
red_band <- c('4-5','4-6','5.1-5','5.1-6','5.2-4','5.2-5','5.2-6')
...
belief_testing <- inner_join(dev_data,belief_testing_rules_disbursals, by=c('deal_no')) 
...
calculate_risk_band_rule_statistics <- function(gating_rules_data, risk_band_temp){
  ...
  for(rule in rule_names){
    temp_data <- gating_rules_subset %>% 
      dplyr::select(c('deal_no', 'bad_loan','ever_90dpd','ever_150dpd', rule))
...
write_xlsx(output_list, "data//output//belief_testing//belief_testing_2W_Used_vf.xlsx")
...
write_xlsx(output_list, "data//output//belief_testing//Summary_belief_testing_2W_Used_vf.xlsx")

save(belief_testing_adj, file = 'data//output//reason_code//2W_New//belief_testing_2W_Used.rdata')
