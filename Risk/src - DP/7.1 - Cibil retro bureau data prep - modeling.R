...
load_rdata_intermediate("cleaned_data//retro_as_on_data_pv_used.rdata")


retro_as_on_data <- data.frame(rbindlist(l=list(retro_as_on_data,retro_as_on_data_pv_used), use.names = T))
...
disbursal_data <- distinct(disbursal_data %>% filter(deal_no %in% unique(bad_loans$deal_no)) %>% dplyr::select(deal_no,
                                                                                                          loan_type,
                                                                                                          disbursal_date,
                                                                                                          year_mon))
...
rm(disbursal_data,retro_as_on_data, bad_loans, retro_as_on_data_pv_used)
...
tradeline_subset$applicant_id <- paste0(tradeline_subset$deal_no, "_", tradeline_subset$customer_code)
...
payment_keys <- unique(trades_clean$payment_key)
closing_date_info <- closing_date_info %>% filter(payment_key %in% payment_keys)
...
save(trades_clean,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "bureau_tradelines_vcheck1.rdata"
     )
)
...
save(trades_clean,
     file = file.path(
       get_data_path()$data$intermediate,
       "ADS_data",
       "bureau_tradelines_dpd_check.rdata"
     )
)
...
load_rdata_intermediate("cleaned_data//enquiry_data.rdata")
load_rdata_intermediate("cleaned_data//enquiry_data_pv_used.rdata")


enquiry_data <- enquiry_data %>% dplyr::select(colnames(enquiry_data_pv_used))

enquiry_data <- data.frame(rbindlist(l = list(enquiry_data,enquiry_data_pv_used), use.names = T))
rm(enquiry_data_pv_used)


## 7.3 subset enquiry data
enquiry_data <- inner_join(enquiry_data, bad_loans, by = "deal_no")
enquiry_data$applicant_id <- paste0(enquiry_data$deal_no,"_",enquiry_data$Customer_Code)
...
trades_clean <- distinct(trades_clean %>% dplyr::select(deal_no,customer_code,applicant_id,disbursal_date,cibil_reported_date))
