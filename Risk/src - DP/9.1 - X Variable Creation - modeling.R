...
variables_PO <- distinct(tradelines %>% dplyr::select(deal_no,customer_code,applicant_id))

for(temp_block in blocks_temp){
  temp_rules <- req_cols[min(temp_block) : max(temp_block)]
  temp_agg <- tradelines %>% dplyr::select(c('deal_no','customer_code','applicant_id',temp_rules))
  temp_agg <- temp_agg %>% group_by(deal_no,customer_code,applicant_id) %>% summarise(across(everything(), list(sum)))
  colnames(temp_agg) <- c('deal_no', 'customer_code','applicant_id', temp_rules)
  temp_agg <- temp_agg %>% ungroup()
  
  variables_PO <- left_join(variables_PO,temp_agg,by = c('deal_no' = 'deal_no',
...
# save(all_X_var_bureau, file = "data//intermediate//ADS_data//X_var_bureau.rdata")
...
req_loan_types <- c('2W-New','PV-New','PV-Used','Refinance-2W')
