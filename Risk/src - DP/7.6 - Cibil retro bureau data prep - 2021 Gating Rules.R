...
load_rdata_intermediate("cleaned_data//retro_as_on_data_2021.rdata")


retro_as_on_data <- data.frame(retro_as_on_data_2021)


## 2. Subset data & create unique loan identifier ------------------------------------------

## 2.3 clean tradeline data
tradeline_subset <- retro_as_on_data_2021 %>% filter((!is.na(high_credit_sanctioned_amount)) &
...
save(trades_clean,
     file = file.path(
       ...
       "ADS_data",
       "bureau_tradelines_2021.rdata"
     )
)



rm(tradeline_subset,retro_as_on_data_2021)
...
trades_clean <- distinct(trades_clean %>% dplyr::select(application_no, customer_code, applicant_id,cibil_reported_date))



enquiry_data <- left_join(enquiry_data_2021,trades_clean, by = c("application_no" = "application_no",
