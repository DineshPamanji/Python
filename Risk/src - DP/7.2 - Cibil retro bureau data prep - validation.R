...
rm(disbursal_data,retro_as_on_data, retro_as_on_data_pv_used)
...
save(trades_clean,
     file = file.path(
       ...
       "ADS_data",
       "bureau_tradelines_validation.rdata"
     )
)
...
trades_clean <- distinct(trades_clean %>% dplyr::select(deal_no,customer_code,applicant_id,disbursal_date,cibil_reported_date,loan_type))
