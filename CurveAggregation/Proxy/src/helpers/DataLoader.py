...
class DataLoader:
    ...
    def get_calendar(self, _week_col, _plan_period_col, _startdate_col, _enddate_col, _planperiodname_col
        ...
        try:
            ...
            self.logger.info(f'Exception message: {str(e)}')
        # calendar[_week_col] = pd.to_datetime(calendar[_week_col], format="%m/%d/%Y")
        # calendar[_week_col] = pd.to_datetime(calendar[_week_col]).dt.strftime("%m/%d/%Y")
        if convert_datetime:
            import datetime as dt
            calendar[_week_col]=calendar[_week_col].apply(lambda x:
    ...
    def get_pop_ratio(self, _geo_col, _pop_col, _ppp_col, parity=0):
        self.input_dict['population'].head()
        del self.input_dict['population']['countryname']
        self.input_dict['population']['key'] = 1
        pop = self.input_dict['population'].copy()
        pop = pop.merge(self.input_dict['population'], on='key', how='left')
        pop['combination'] = pop[_geo_col + '_x'] + " - " + pop[_geo_col + '_y']
        pop['pop_ratio'] = pop[_pop_col + '_x'] / pop[_pop_col + '_y']
        keep_cols = ['combination', 'pop_ratio']
        if parity:
            pop['ppp_ratio'] = pop[_ppp_col + '_x'] / pop[_ppp_col + '_y']
            keep_cols = keep_cols + ['ppp_ratio']
        pop = pop.loc[:, keep_cols]
        return pop
...
class CalculationEngine:
    ...
    def transform_curves_format(self, df, curves_4
                                ...
                                , curves_consolidated, _state_level=0):
        curves_1 = df.loc[:, [_geo_col, _brand_col, _instrument_col, _coeffA_col, _coeffB_col, _coeffC_col]]
        if _state_level:
            curves_1['key']=1
            self.input_dict['states']['key']=1
            curves_1 = curves_1.merge(self.input_dict['states'], how='left')
            del curves_1['key']
        else:
            curves_1[_state_col] = '-'
        ...
        curves_1 = curves_1.merge(curves_consolidated[dimension_cols+r_dimension_cols+
                                                      [_instrument_col]].drop_duplicates(), how='left')
        for col in dimension_cols:
            rcol = "receiving" + col
            curves_1.loc[curves_1[rcol].isnull(), rcol] = curves_1.loc[curves_1[rcol].isnull(), col]
        ...
        curves_1['isdeleted'] = 'FALSE'
        curves_1['isnonmeasured'] = 'FALSE'
        ...
        curves_1.rename(columns={'spend_type':'spendtype'}, inplace=True)
        curves_1['newinstrumentname'] = '-'
    ...
    def get_kpi_master(self, curves_t, kpi_template_df, _brand_col, _subbrand_col, _geo_col, _state_col
        ...
        final_kpi = final_kpi.merge(curves_consolidated[dimension_cols+r_dimension_cols+[_instrument_col]].drop_duplicates(), how='left')
        for col in dimension_cols:
            rcol = "receiving" + col
            final_kpi.loc[final_kpi[rcol].isnull(), rcol] = final_kpi.loc[final_kpi[rcol].isnull(), col]
    ...
    def get_LT_EU(self, ETL_kpi, final_kpi,  _brand_col, _geo_col, _instrument_col, _region_col):
        ...
        final_kpi_LT_EURO_values = final_kpi_LT_EURO_t[~np.isnan(final_kpi_LT_EURO_t['lt'].astype('float'))]
        final_kpi_LT_EURO_missing = final_kpi_LT_EURO_t[np.isnan(final_kpi_LT_EURO_t['lt'].astype('float'))]

        self.logger.info('If a combination is not available, the average instrument level must be used')
        lt_values_df_eu['lt'] = lt_values_df_eu['lt'].replace('',np.nan).astype(float)
    ...
    def get_finance_KPI(CalcEngine, final_kpi, _brand_col, _geo_col, _region_col
        ...
        CalcEngine.input_dict['finance'] = CalcEngine.input_dict['finance'][CalcEngine.input_dict['finance'][_nsv_per_eu_col] > 0]
