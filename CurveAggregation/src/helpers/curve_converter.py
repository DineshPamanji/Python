...
class CurveConverter:
    ...
    def eval_curve_multi(self, df, output_field, form_col):
        # self.df[output_field] = self.df.eval(self.get_curve_equation(curve_name))
        df[output_field] = np.vectorize(self.curve_eval)(df['coefficienta'], df['coefficientb']
                                                         , df['coefficientc'], df['adstock'], df[form_col])
        return df
    ...
    def minimize_options(self, coeffs_list, power=0, nobounds=0):
        new_coeffs_list = []
    ...
    def convert_curve(self, curves_4, coeffs, form_col,  _spend_mean_col, _stimuli_col, curve_row):
        ...
        self.create_adstock(stimuli, curve_row, ntimes=5, step=1, step2=30)
    ...
    def convert_curve_group(self, curves_4, coeffs, form_col,  _spend_mean_col, _stimuli_col, i, j):
        results = []
        comparison_df_final = []
        for curve_row in range(i, j):
            new_coeffs, comparison_df = self.convert_curve(curves_4, coeffs, form_col, _spend_mean_col, _stimuli_col, curve_row)
            results.append(new_coeffs)
            comparison_df_final.append(comparison_df)
        return results, comparison_df_final

    def calculate_ad_stocks_vectorized(self, data, engagement_factor):
