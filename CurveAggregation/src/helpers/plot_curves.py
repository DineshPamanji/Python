...
time = '2021-04-14'
bus_inputs_3_all = pd.read_excel(path+f'\Outputs\AggregatedCurves\{time}_Curves_Aggregated.xlsx', index=False)
comparison_df_final = pd.read_excel(path+f'\Outputs\AggregatedCurves\{time}_Aggregated_Curves_comparison_df_final.xlsx')

for curve_row in range((len(comparison_df_final['CurveID'].unique()))):
    # curve_row = 0
    # curve_row = 251
    ...
    curve_name = 'ADBUDG'#converter.get_curve_name(curve_row, form_col)
    bcv_curve = '-'.join(final_response_curves_id.loc[curve_row, dimension_cols].tolist())#curves_4_final['Selection'][curve_row].replace("/", " ")
    ...
    plt.savefig(path+f'\Outputs\AggregatedCurves\Plots\{curve_row+10000}_{str(bcv_curve)}.png')

comparison_df_final_93 = comparison_df_final[comparison_df_final['CurveID']==93]
curves_4_final.iloc[93,:]
comparison_df_final_93.to_csv(path+'\Outputs\AggregatedCurves\Plots\Converted_Curves_comparison_df_final_93.csv', index=False)
